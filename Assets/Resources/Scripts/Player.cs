...
public class Player : Personagem {
	...
	void Start () {
		...
		if (sceneManager.checkpointBoss) {
			transform.position = sceneManager.checkpointCoordBoss;
        } else if (sceneManager.checkpoint) {
			transform.position = sceneManager.checkpointCoord;
        } else {
			transform.position = sceneManager.startCoord;
        }
	}

	void FixedUpdate() {
		if (!sceneManager.cutscene) {
			if (!isDead) {
				UpdateDirecoes ();
				IsGrounded ();
	...
	// Espera uma quantidade de tempo até resetar a fase
	IEnumerator WaitTime()
    {
        yield return new WaitUntil(() => frame >= qtdFrames);
		...
		if (Game.qtdVidas == 0) {
			...
			if (sceneManager.cutsceneBeforeBoss) {
				sceneManager.cutsceneBeforeBossOk = true;
			}
	...
	private void OnCollisionEnter2D(Collision2D col) {
		if (col.gameObject.name.Equals("moving_platform")) {
